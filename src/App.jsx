...
// ─── Cat renderer ─────────────────────────────────────────────────────────────
function drawCat(ctx, x, y, p, frame, pal, flip, state, grabbed, isOwn, isSys) {
  ctx.save();
  ctx.translate(x, y);
  ...
  if (blink) {
    ctx.fillStyle = "#000"; ctx.fillRect(2*p,(bob+1)*p,3*p,p); ctx.fillRect(6*p,(bob+1)*p,3*p,p);
  } else {
...
function drawBubble(ctx, cat, p) {
  ...
  ctx.save(); ctx.globalAlpha=cat.revealAlpha;
  ctx.font=`${Math.max(7,p*1.8)}px 'Press Start 2P',monospace`;
  const tw=ctx.measureText(text).width;
  const pad=10,bh=26,bw=tw+pad*2;
...
function loadSession(){
  try{
    const r=localStorage.getItem("fikfuk_s");
    if(!r) return null;
    const s=JSON.parse(r);
    if(s.epoch===getEpoch()) return s;
  }catch(e){}
  return null;
}
...
// ─── Cat factory ──────────────────────────────────────────────────────────────
let uid=100;
function makeCat(x,y,palOrObj,name,isOwn=false,isSys=false,sysType=null){
  const palObj = typeof palOrObj==="number"
    ? {...PALETTES[palOrObj], _name:name}
    : {...palOrObj};
  if(!palObj._name) palObj._name=name;
  return {
    id:uid++, x, y, vx:0, vy:0,
...
export default function App(){
  ...
  const gs = useRef({ // game state - never triggers re-render
  ...
  // ── Utility: build cats from W/H ──────────────────────────────────────────
  function buildCats(W, H, name, palId, restored){
    const cats=[];
    const m=80;

    // system cats
    const pc = makeCat(W*0.15+Math.random()*W*0.2, m+Math.random()*(H-m*2), SYS_PAL_PRIVACY,"policy",false,true,"privacy");
    pc.confessions=[...PRIVACY_LINES];
    const ac = makeCat(W*0.6+Math.random()*W*0.25, m+Math.random()*(H-m*2), SYS_PAL_ABOUT,"about",false,true,"about");
    ...
    return { cats, myId: myCat.id };
  }

  // ── enterPlay: called AFTER phase="play" renders the canvas ───────────────
  // We set phase first, then use a useEffect that watches phase to do the rest.
  ...
  // This fires after "play" phase renders and canvas is in the DOM
  useEffect(()=>{
    if(phase !== "play") return;
    if(!pendingSpawn.current && !gs.current.ready) return;

    // Give browser one frame to paint the canvas element
    const raf = requestAnimationFrame(()=>{
      ...
      if(pendingSpawn.current){
        ...
        const {cats,myId}=buildCats(W,H,name,palId,restored);
        s.cats=cats; s.myId=myId; s.ready=true;
        saveSession({name,palId,catId:myId,confessions:[],x:cats.find(c=>c.id===myId).x,y:cats.find(c=>c.id===myId).y});
      }
    });
  ...
  useEffect(()=>{
    ...
    if(sess){
      requestSpawn(sess.name, sess.palId, {confessions:sess.confessions, x:sess.x, y:sess.y});
    } else {
      setPhase("onboard");
  ...
  useEffect(()=>{
    const iv=setInterval(()=>{
      ...
      if(ms<=1000){
        ...
        s.cats=[]; s.myId=null; s.laserOn=false; s.ready=false;
        s.laser={x:-999,y:-999,active:false}; s.trail=[];
        pendingSpawn.current=null;
        setLaserOn(false);
        phaseRef.current="onboard";
        setPhase("onboard");
  ...
  // ── Game loop (starts when phase becomes play) ────────────────────────────
  useEffect(()=>{
    if(phase!=="play") return;
    const cv=canvasRef.current;
    if(!cv) return;
    ...
    const resize=()=>{
      cv.width=cv.offsetWidth||window.innerWidth;
      cv.height=cv.offsetHeight||window.innerHeight;
      gs.current.stars=genStars(cv.width,cv.height);
    };
    ...
    const tick=()=>{
      ...
      s.cats.forEach(cat=>{
        ...
        const inRange=cat.isOwn&&!cat.isSys&&laser.active&&dist<cat.range;
        ...
        if(inRange){
          ...
          if(cat.wanderTimer<=0){
            ...
            if(!cat.isSys) cat.state=Math.random()>0.45?"sit":"run";
          }
          if(cat.state==="run"&&!cat.isSys){
            cat.vx+=Math.cos(cat.wanderAngle)*0.03;
            cat.vy+=Math.sin(cat.wanderAngle)*0.03;
            cat.flip=cat.vx<0;
          }
        }
      ...
      if(s.frame%300===0){
        const mc=s.cats.find(c=>c.id===s.myId);
        const sess=loadSession();
        if(sess&&mc) saveSession({...sess,x:mc.x,y:mc.y,confessions:mc.confessions});
      }
      ...
      s.cats.forEach(c=>{
        ...
        drawCat(ctx,c.x,c.y,P,c.frame,c.pal,c.flip,c.state,c.grabbed,c.isOwn,c.isSys);
      });
  ...
  // ─── Render ───────────────────────────────────────────────────────────────
  const panelAccent = panel?.isSys
    ? (panel.sysType==="privacy"?"#7777ff":"#44cc44")
    : (panel?.isOwn?"#ff3232":"rgba(120,120,255,0.7)");

  return(
    <div style={{width:"100%",height:"100vh",background:"#080810",position:"relative",overflow:"hidden",fontFamily:ff}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .hbtn{transition:background .15s,box-shadow .15s;cursor:pointer;}
        .hbtn:hover{background:rgba(255,50,50,0.2)!important;box-shadow:0 0 20px rgba(255,50,50,0.55)!important;}
      `}</style>

      {/* Canvas — always mounted so ref is always valid */}
      <canvas
        ref={canvasRef}
        style={{
          ...
          // visually hidden during onboard/loading
        ...
        <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"#080810",zIndex:100,padding:20}}>
          <div style={{background:"#0d0d1a",border:"2px solid #ff3232",boxShadow:"0 0 40px rgba(255,50,50,0.2)",padding:28,maxWidth:360,width:"100%",display:"flex",flexDirection:"column",gap:22}}>
            ...
            <div>
              ...
              <input
                maxLength={12} value={pickName} autoFocus
            ...
            <div>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {PALETTES.map(p=>(
                  <div key={p.id} className="pal-swatch"
                    onClick={()=>setPickPal(p.id)}
                    style={{
                      width:34,height:34,background:p.body,position:"relative",
                      border:`2px solid ${pickPal===p.id?"#ff3232":"rgba(255,255,255,0.1)"}`,
                      boxShadow:pickPal===p.id?"0 0 14px rgba(255,50,50,0.65)":"none",
                    }}>
            ...
            <button className="hbtn"
              ...
              style={{
                fontFamily:ff,fontSize:8,padding:"13px",
                background:pickName.trim()?"rgba(255,50,50,0.18)":"rgba(30,30,50,0.5)",
                border:`2px solid ${pickName.trim()?"#ff3232":"rgba(255,50,50,0.2)"}`,
                color:pickName.trim()?"#ff7777":"rgba(255,100,100,0.3)",
                boxShadow:pickName.trim()?"0 0 14px rgba(255,50,50,0.3)":"none",
                cursor:pickName.trim()?"pointer":"not-allowed",
                letterSpacing:1,
              }}>
              spawn my cat →
            </button>
      ...
      {/* ── PLAY HUD ── */}
      {phase==="play"&&(<>
        <div style={{position:"absolute",top:14,left:14,zIndex:10,pointerEvents:"none",display:"flex",gap:14,alignItems:"center"}}>
        ...
        <div style={{position:"absolute",bottom:22,right:22,display:"flex",gap:10,alignItems:"center",zIndex:20}}>
          <button className="hbtn" onClick={toggleLaser} style={{
            fontFamily:ff,fontSize:8,
            background:laserOn?"rgba(255,50,50,0.22)":"transparent",
            border:`2px solid ${laserOn?"#ff3232":"rgba(255,50,50,0.45)"}`,
            color:laserOn?"#ff4444":"rgba(255,100,100,0.6)",
            padding:"10px 13px",
            boxShadow:laserOn?"0 0 18px rgba(255,50,50,0.5)":"none",
          }}>
            {laserOn?"🔴 laser on":"⭕ laser off"}
          </button>

          {!showInput&&(
            <button className="hbtn" onClick={()=>{if(cooldown===0)setShowInput(true);}} style={{
              fontFamily:ff,fontSize:8,
              background:cooldown>0?"rgba(40,40,60,0.6)":"transparent",
              border:`2px solid ${cooldown>0?"rgba(255,50,50,0.25)":"#ff3232"}`,
              color:cooldown>0?"rgba(255,100,100,0.4)":"#ff6666",
              padding:"10px 14px",
              boxShadow:cooldown>0?"none":"0 0 14px rgba(255,50,50,0.35)",
        ...
        {showInput&&(
          <div style={{position:"absolute",bottom:22,right:22,left:22,background:"rgba(8,8,16,0.97)",border:"2px solid #ff3232",boxShadow:"0 0 30px rgba(255,50,50,0.25)",padding:"16px",zIndex:30,display:"flex",flexDirection:"column",gap:10}}>
      ...
      {/* ── Panel (any phase) ── */}
      {panel&&(
        <div style={{position:"absolute",inset:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.65)"}}
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:"#0d0d1a",border:`2px solid ${panelAccent}`,boxShadow:`0 0 40px ${panelAccent}55`,padding:"20px",minWidth:280,maxWidth:"90vw",display:"flex",flexDirection:"column",gap:12}}>
            ...
            {panel.sysType==="about"&&(
              <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:4}}>
                ...
                <div
                  onClick={()=>navigator.clipboard?.writeText("3oDULkLmFSXppKGyKLQjE32MSSmDcyAcZL2jWy94rbp2")}
                  style={{
                    fontFamily:"monospace",fontSize:7,color:"rgba(100,255,100,0.7)",
                    background:"rgba(0,255,0,0.06)",border:"1px solid rgba(68,204,68,0.25)",
                    padding:"8px",wordBreak:"break-all",lineHeight:1.8,
